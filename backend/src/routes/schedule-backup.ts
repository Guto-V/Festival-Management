...
router.post('/performance', authenticateToken, requireMinimumRole('coordinator'), async (req: AuthenticatedRequest, res) => {
  try {
    ...
    if (!artist_id || !venue_id || !performance_date || !start_time || !duration_minutes) {
    ...
    const endMinutes = startMinutes + validatedDuration + changeover_time_after;
    ...
    const conflicts = await db.all(`
      ...
      FROM performances p
      JOIN artists a ON p.artist_id = a.id
      WHERE p.venue_id = ? AND p.performance_date = ? AND p.status != 'cancelled'
      AND (
        (? >= start_minutes AND ? < start_minutes + p.duration_minutes + p.changeover_time_after)
        OR
        (start_minutes >= ? AND start_minutes < ?)
      )
    `, [venue_id, performance_date, startMinutes, startMinutes, startMinutes, endMinutes]);
    ...
    const result = await db.run(`
      INSERT INTO performances (
        festival_id, artist_id, venue_id, performance_date, start_time,
        duration_minutes, changeover_time_after, soundcheck_time,
        soundcheck_duration, notes, status
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `, [
      festival_id, artist_id, venue_id, performance_date, start_time,
      validatedDuration, changeover_time_after, soundcheck_time,
      soundcheck_duration, notes, status
    ]);
    ...
    const newPerformance = await db.get(`
      SELECT 
        p.id, p.festival_id, p.artist_id, p.venue_id, p.performance_date,
        p.start_time, p.duration_minutes, p.changeover_time_after,
        p.soundcheck_time, p.soundcheck_duration, p.notes, p.status,
        a.name as artist_name, a.genre,
...
router.put('/performance/:id', authenticateToken, requireMinimumRole('coordinator'), async (req: AuthenticatedRequest, res) => {
  try {
    const { id } = req.params;
    const {
      artist_id, venue_id, performance_date, start_time, duration_minutes,
    ...
    if (start_time || duration_minutes || venue_id || performance_date || changeover_time_after) {
      const checkStartTime = start_time || performance.start_time;
      const checkDuration = validatedDuration || performance.duration_minutes;
      const checkVenueId = venue_id || performance.venue_id;
      const checkDate = performance_date || performance.performance_date;
      const checkChangeover = changeover_time_after !== undefined ? changeover_time_after : performance.changeover_time_after;

      const startMinutes = timeToMinutes(checkStartTime);
      const endMinutes = startMinutes + checkDuration + checkChangeover;
      ...
      const conflicts = await db.all(`
        ...
        FROM performances p
        JOIN artists a ON p.artist_id = a.id
        WHERE p.venue_id = ? AND p.performance_date = ? AND p.status != 'cancelled' AND p.id != ?
        AND (
          (? >= start_minutes AND ? < start_minutes + p.duration_minutes + p.changeover_time_after)
          OR
          (start_minutes >= ? AND start_minutes < ?)
    ...
    await db.run(`
      ...
      WHERE id = ?
    `, [
      artist_id, venue_id, performance_date, start_time, validatedDuration,
      changeover_time_after, soundcheck_time, soundcheck_duration, notes, status, id
    ]);
...
// Add changeover time block
router.post('/changeover', authenticateToken, requireMinimumRole('coordinator'), async (req: AuthenticatedRequest, res) => {
  try {
    const {
      festival_id = 1,
      venue_id,
      ...
      start_time,
      duration_minutes = 15,
      notes = 'Changeover time'
    } = req.body;

    if (!venue_id || !performance_date || !start_time) {
      return res.status(400).json({ error: 'Missing required fields' });
    }
    ...
    const changeoverArtist = await db.get(
      'SELECT id FROM artists WHERE name = "CHANGEOVER"'
    );

    let changeoverArtistId;
    if (!changeoverArtist) {
      const result = await db.run(
        'INSERT INTO artists (name, status) VALUES ("CHANGEOVER", "confirmed")'
      );
      changeoverArtistId = result.lastID;
    } else {
      changeoverArtistId = changeoverArtist.id;
    }

    const result = await db.run(`
      INSERT INTO performances (
        festival_id, artist_id, venue_id, performance_date, start_time,
        duration_minutes, changeover_time_after, notes, status
      ) VALUES (?, ?, ?, ?, ?, ?, 0, ?, 'confirmed')
    `, [festival_id, changeoverArtistId, venue_id, performance_date, start_time, duration_minutes, notes]);

    const performanceId = result.lastID;

    // Get the created changeover block
    const newChangeover = await db.get(`
      SELECT 
        p.id, p.festival_id, p.artist_id, p.venue_id, p.performance_date,
        p.start_time, p.duration_minutes, p.changeover_time_after,
        p.notes, p.status,
    ...
    res.status(201).json(newChangeover);
  } catch (error) {
    console.error('Create changeover error:', error);
    res.status(500).json({ error: 'Internal server error' });
  }
});
